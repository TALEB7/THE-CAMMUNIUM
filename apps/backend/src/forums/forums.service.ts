...
export class ForumsService {
  ...
  async deleteForum(id: string) {
    // Delete all posts and comments first to respect foreign keys
    await this.prisma.forumComment.deleteMany({ where: { post: { forumId: id } } });
    await this.prisma.forumPost.deleteMany({ where: { forumId: id } });
    return this.prisma.forum.delete({ where: { id } });
  }

  // ── Posts ──

  async getForumPosts(forumId: string, pageParam: any = 1, limitParam: any = 20) {
